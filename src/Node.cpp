...
/* DEBUG */ log(INFO) << "DEBUG - OpenCL platform: '" << automy::basic_opencl::get_platform_name(idp) << "' (" << idp << ")";

			const auto devices = automy::basic_opencl::get_devices(idp, CL_DEVICE_TYPE_GPU);
			for(const auto idd : devices) {

/* DEBUG */ log(INFO) << "DEBUG - OpenCL device: '" << automy::basic_opencl::get_device_name(idd) << "' (" << idd << ")" << "[ " << "'" << automy::basic_opencl::get_platform_name(idp) << "' (" << idp << ")" << " ]";
